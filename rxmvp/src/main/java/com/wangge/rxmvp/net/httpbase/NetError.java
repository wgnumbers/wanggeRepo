...
public class NetError {
    ...
    public static <T> NetError buildError(@NonNull Response<T> response) {
        try {
            NetError error = new Gson().fromJson(response.errorBody().string(), NetError.class);
            if (error != null ){
    ...
    public static NetError buildError(@NonNull Throwable t) {
        NetError error = new NetError();
        error.success = false;
